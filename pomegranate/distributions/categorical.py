...
class Categorical(Distribution):
    ...
    def _reset_cache(self):
        ...
        self.register_buffer("_log_probs", torch.log(self.probs + EPS))
    ...
    def log_probability(self, X):
        ...
        for i in range(self.d):
            if isinstance(X, torch.masked.MaskedTensor):
                ...
                check_for_nans_infs(logp_, "logp_ log_probability categorical 1")
                logps += logp_
            else:
                # check_for_nans_infs(self._log_probs[i][X[:, i]], "self._log_probs[i][X[:, i]] log_probability categorical 1")
                logps += self._log_probs[i][X[:, i]]

        check_for_nans_infs(logps, "logps log_probability categorical 1")
        return logps
