...
def _check_inputs(model, X, emissions, priors):
    ...
    check_for_nans_infs(emissions, "emissions _check_inputs _base 1")
    emissions = _check_parameter(_cast_as_tensor(emissions), "emissions", 
        ndim=3)
    check_for_nans_infs(emissions, "emissions _check_inputs _base 2")
    if emissions is None:
        emissions = model._emission_matrix(X, priors=priors)
        check_for_nans_infs(emissions, "emissions _check_inputs _base 3")
...
class _BaseHMM(Distribution):
    ...
    def _emission_matrix(self, X, priors=None):
        ...
        for i, node in enumerate(self.distributions):
            ...
            check_for_nans_infs(logp, "logp _emission_matrix _base 2")
            if isinstance(logp, torch.masked.MaskedTensor):
                logp = logp._masked_data

            e[:, i] = logp.reshape(n, k).T
        
        check_for_nans_infs(e, "e _emission_matrix _base 2")

        e = e.permute(2, 0, 1)
        if priors is not None:
            e += torch.log(priors+ EPS)

        check_for_nans_infs(e, "e _emission_matrix _base 3")
        return e
    ...
    def log_probability(self, X, priors=None):
        ...
        """
        check_for_nans_infs(X, "X log_probability _base 1")
        f = self.forward(X, priors=priors)
        check_for_nans_infs(f, "f log_probability _base 1")
        check_for_nans_infs(self.ends, "self.ends log_probability _base 1")
        result = torch.logsumexp(f[:, -1] + self.ends, dim=1)
        check_for_nans_infs(result, "result log_probability _base 1")
    ...
    def summarize(self, X, sample_weight=None, emissions=None, 
        ...
        check_for_nans_infs(emissions,"emissions (summarize base 1)")
        X = _check_parameter(_cast_as_tensor(X), "X", ndim=3, 
            shape=(-1, -1, self.d), check_parameter=self.check_data)
        
        emissions = _check_inputs(self, X, emissions, priors)
        check_for_nans_infs(emissions,"emissions (summarize base 2)")
    ...
    def from_summaries(self):
        ...
        raise Exception("Infinite loop in from_summaries")
